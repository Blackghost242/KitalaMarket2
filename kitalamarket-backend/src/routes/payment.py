...
@payment_bp.route('/mtn/request-to-pay', methods=['POST'])
def mtn_request_to_pay():
    ...
    try:
        ...
        mtn_response = simulate_mtn_request_to_pay(
        ...
        if not mtn_response['success']:
            return jsonify({
                'error': mtn_response['error'],
                'message': mtn_response['message']
            }), 400
        
        # Store payment information in session or database
        payment_data = {
            'transaction_id': mtn_response['transaction_id'],
            'external_id': external_id,
            'order_id': order_id,
            'amount': order.total_price,
            'phone_number': phone_number,
        ...
        session['pending_payments'][mtn_response['transaction_id']] = payment_data
        ...
        return jsonify({
            ...
            'amount': order.total_price,
            'status': 'PENDING',
            'instructions': 'Vérifiez votre téléphone et confirmez le paiement'
        }), 200
        
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@payment_bp.route('/mtn/status/<transaction_id>', methods=['GET'])
